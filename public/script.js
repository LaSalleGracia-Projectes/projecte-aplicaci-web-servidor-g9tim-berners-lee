...
async function cargarTendencias() {
    ...
    try {
        const res = await fetch(
            `${BASE_URL}/trending/movie/week?api_key=${API_KEY}&language=es`
        );
        const data = await res.json();
...
async function renderFavorites() {
    ...
    for (const id of favorites) {
        try {
            const res = await fetch(
                `${BASE_URL}/movie/${id}?api_key=${API_KEY}&language=es`
            );
...
async function openMovieDetail(movieId) {
    ...
    try {
        ...
        <p><strong>Rating:</strong> ${movie.vote_average} (${renderStars(
            movie.vote_average
        )})</p>
        <p>${movie.overview}</p>
        ...
        <a href="/infoPelicula/${
            movie.id
        }" class="action-btn">Ver más detalles</a>
      `;
...
function renderRandomContent(content, tipo) {
    ...
    container.innerHTML = `
    <img src="${IMG_URL}${
        tipo === "movie" ? content.poster_path : content.backdrop_path
    }" alt="${content.title || content.name}" width="150">
    <h3>${content.title || content.name}</h3>
    <p>${content.overview.substring(0, 100)}...</p>
    <button class="action-btn" onclick="openMovieDetail(${
        content.id
    })">Ver Detalle</button>
  `;
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("registerForm").addEventListener("submit", function (event) {
        ...
        console.log({ name, email, password, password_confirmation }); // 🔍 Verificar en consola
        ...
        fetch("/api/register", {
            ...
            .then((data) => {
                console.log("Respuesta del servidor:", data); // 🔍 Ver error exacto en consola
    ...
    // Capturar el formulario de login dentro del modal
    document.getElementById("loginForm").addEventListener("submit", function (event) {
        ...
        console.log({ email, password }); // 🔍 Verificar en consola
...
document.getElementById("spoilerBtn").addEventListener("click", () => {
    if (
        confirm(
            "Al hacer clic en los perfiles de críticos podrías ver spoilers. ¿Deseas continuar?"
        )
    ) {
...
document
    .getElementById("generarRandom")
    .addEventListener("click", generarRecomendacion);
